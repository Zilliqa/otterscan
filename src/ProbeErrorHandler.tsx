...
const ProbeErrorHandler: FC<ProbeErrorHandlerProps> = ({config} : ProbeErrorHandlerProps) => {
  const err = useAsyncError();
  if (!(err instanceof ProbeError)) {
    throw err;
  }
  return <ConnectionErrorPanel connStatus={err.status} nodeURL={err.nodeURL} config={config} />;
};
