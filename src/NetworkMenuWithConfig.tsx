...
const NetworkMenuWithConfig: FC<NetworkMenuWithConfigProps> = ({ config }) => {
  ...
  return (
    ...
    <Menu>
      ...
      {connectionItems}
      <NetworkSetItem onClick={() => setGoToOpen(1)}/>
      </MenuItems>
      </div>
   </Menu>
      { goToOpen && (
        ...
        >
          ...
          URL: <input id="url" className="min-w-[40ch] border ml-4 border-gray-400" onChange={(e) => setConnectUrl(e.target.value)}></input>
          </div>
          </div>
        ...
        type="button"
                  className="mt-3 w-full inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm"
        onClick={() => { if (!newNetwork(connectName, connectUrl)) {  alert('Connection not found?!') }; setGoToOpen(false);} }
          >
          Connect
          </button>
              </div>
  ...
  <MenuItem>
    {({ focus }) => (
      <div
        ...
        >
        <button name="Connect" onClick={onClick}>Connect</button>
      </div>
    )}
    </MenuItem>);
}
